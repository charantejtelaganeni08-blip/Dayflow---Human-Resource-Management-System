...
interface HRDataValue {
  ...
  updatePayslip: (payslipId: string, structure: SalaryStructure) => void;
...
export function HRDataProvider({ children }: {children: React.ReactNode;}) {
  ...
  const updatePayslip = useCallback(
    (payslipId: string, structure: SalaryStructure) => {
      const payslip = payslips.find((slip) => slip.id === payslipId);

      if (!payslip) {
        throw new Error('The selected payslip could not be found.');
      }

      setPayslips((current) =>
        current.map((slip) =>
          slip.id === payslipId ? { ...slip, structure } : slip
        )
      );
      ...
      if (payslip.published) {
        pushNotification(
          payslip.employeeId,
          'Payslip revised',
          `Your ${payslip.month} payslip was revised by HR. Open payslips to review it.`,
          '/payslips'
        );
      }
    },
    [payslips, pushNotification]
  );
